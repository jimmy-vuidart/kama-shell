...
- Une surface permanente sur `WlrLayer.Overlay` reste au-dessus des fenêtres fullscreen dans niri. Pour un décor de shell qui doit disparaître pendant les jeux/vidéos plein écran, utiliser `WlrLayer.Top` et vider aussi le `mask`/`BackgroundEffect.blurRegion` quand une fenêtre de l'output actif est fullscreen ou couvre entièrement l'output.
- `niri msg --json windows` peut ne pas exposer de flag fullscreen explicite pour certains jeux borderless/fullscreen-like; comparer `layout.window_size` à la taille logique de l'output actif permet de détecter ces cas sans heuristique par application.
