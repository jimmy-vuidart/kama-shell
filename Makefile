...
.PHONY: run check test fmt help install-session-niri install-session-niri-debug
...
TEST_RUNTIME_DIR ?= /tmp/kama-shell-test-runtime
...
test: check
	...
	$(PYTHON) scripts/check-qml-asset-urls.py
	mkdir -p $(TEST_RUNTIME_DIR)
	env XDG_RUNTIME_DIR=$(TEST_RUNTIME_DIR) QT_QPA_PLATFORM=offscreen timeout 5s $(QUICKSHELL) -p src/RingPathSelfTest.qml 2>&1 | grep -q "RingPathSelfTest passed"
